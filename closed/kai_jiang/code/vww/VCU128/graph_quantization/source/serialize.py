...
class ModelData:
    ...
    def create_txt(self, prefix, save):
        # position data
    ...
    def create_source(self, prefix, save=False):
        ...
        if (self.averagepool_pos.kernel_shape_pos):
            ...
            stride_0 = self.config[self.averagepool_pos.strides_pos[0]]
            stride_1 = self.config[self.averagepool_pos.strides_pos[0] + 1]
            assert stride_0 == stride_1, "average pool stride should be same!"
            shift_value = get_shift_n(1 / (stride_0 * stride_1))

            cm += "const int AVERAGE_POOLING_DIVISOR[] = {" +"{}, {}".format(shift_value[0], shift_value[1]) + "};\n"
