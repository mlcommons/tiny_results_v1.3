...
def inference_calibration_activations(data_path, data, graph, mode="kl",
                                        percentile=99.99, number=20, input_range=1):
    """inference to generate calibration activations
    """
    activations = graph.activations
    all_outputs = {}
    ...
    if data is not None:
        for key, elem_data in data.items():
            if count_file == number:
                break
            count_file += 1
            elem_data = preprocess_cifar(elem_data)
            calibration_data.append(elem_data)
    ...
    for elem_data in calibration_data:
        output = inference_onnx(elem_data, graph.onnx_model)
        for activation_name, activation_value in output:
            if activation_name in activations:
                if activation_name in all_outputs:
                    all_outputs[activation_name] = np.concatenate((all_outputs[activation_name], activation_value.flatten()))
    ...
    factors = {}
    bit_width = Type2bitwidth[ActivationQuantizationType]
    quantization_range = pow(2, bit_width) - 1
    
    factors["input"] = (Q.get_quantizing_factor(input_range, INPUTQuantizationType), input_range, 0)  # set input factor
