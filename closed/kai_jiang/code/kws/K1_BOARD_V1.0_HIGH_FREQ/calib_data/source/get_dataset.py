...
def get_training_data(Flags, get_waves=False, val_cal_subset=True):
  ...
  (ds_train, ds_test, ds_val), ds_info = tfds.load('speech_commands', split=splits, 
                                                data_dir=Flags.data_dir, with_info=True, download=True)

  if val_cal_subset:  # only return the subset of val set used for quantization calibration
    print("calibration subset")
    with open("source/quant_cal_idxs.txt") as fpi:
...
if __name__ == '__main__':
  ...
  i = 0
  cal_file_path = Flags.cal_file_path
  print(f"cal_file_path: {cal_file_path}")
  if not os.path.exists(cal_file_path):
    os.makedirs(cal_file_path)
  ds_val = ds_val.unbatch()
  images = list(ds_val.map(lambda x, y: x))
  labels = list(ds_val.map(lambda x, y: y))

  for i in range(0,len(images)):
    sample_img = np.expand_dims(np.array(images[i], dtype=np.float32), axis=0)
    f = open(cal_file_path + "/sample_%s.bin" %i, "wb") 
    f.write(sample_img)
    f.close()
    i+=1
